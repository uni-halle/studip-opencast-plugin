...
class OCModel
{
    ...
    static function getOCRessources() {
       $stmt = DBManager::get()->prepare("SELECT * FROM resources_objects ro
                ...
                WHERE rop.property_id IN (SELECT property_id FROM resources_properties WHERE name = 'automatische Vorlesungsaufzeichnung' )
                AND rop.state = 'on'");
    ...
    static function getAssignedOCRessources() {
       $stmt = DBManager::get()->prepare("SELECT * FROM resources_objects ro
                ...
                WHERE rop.property_id = (SELECT property_id FROM resources_properties WHERE name = 'automatische Vorlesungsaufzeichnung' )
                AND rop.state = 'on'");
